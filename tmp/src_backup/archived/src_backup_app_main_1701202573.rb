...
class TetrisGame
  ...
  def render_grid_border
    ...
    for i in x...(x + grid_border_width) do
      render_cube i, y, 255, 0, 0
      render_cube i, y + grid_border_height - 1, 255, 0, 0
    end

    for i in y...(y + grid_border_height) do
      render_cube x, i, 255, 0, 0
      render_cube x + grid_border_width - 1, i, 255, 0, 0
    end
  end
  ...
  def render_grid
    for x in 0...@grid_width do
      for y in 0...@grid_height do
        render_cube(x, y, , 0, 0) if @grid[x][y] != 0
      end
    end
  end
